...
export const WorkbenchTab = () => {
  ...
  const refreshTables = async () => {
    ...
    try {
      ...
      const data = r.data || [];
      setTables(data);
      setRefreshMsg(`${data.length} table${data.length !== 1 ? 's' : ''} loaded`);
      setTimeout(() => setRefreshMsg(null), 3000);
    } catch (err: any) {
      const detail = err?.response?.data?.detail || err?.message || 'Could not reach server';
      setRefreshMsg(`Error: ${detail}`);
                ...
                <>
                  <div className="p-3 border-b border-border space-y-2">
                    ...
                    <button
                      onClick={refreshTables}
                      disabled={refreshing}
                      className="w-full btn btn-ghost text-[11px] py-1.5 px-3 flex items-center justify-center gap-1.5 disabled:opacity-50"
                    >
                      <RefreshCw size={11} className={refreshing ? 'animate-spin' : ''} />
                      {refreshing ? 'Refreshing...' : 'Refresh Tables'}
                    </button>
                    {refreshMsg && (
                      <p className={`text-[10px] text-center font-mono px-2 pb-1 ${refreshMsg.startsWith('Error') ? 'text-red-500' : 'text-green-600'}`}>
                        {refreshMsg}
                      </p>
                    )}
                  </div>
