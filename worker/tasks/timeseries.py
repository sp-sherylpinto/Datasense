...
def _apply_month_filter(df: pl.LazyFrame, date_col: str, filter_months: list[int]) -> pl.LazyFrame:
    """Filter rows to only those whose date_col month is in filter_months.
    Handles TIMESTAMPTZ, plain Date, and string formats.
    Collects a tiny sample first to detect real dtype — never guesses.
    """
    try:
        sample = df.select(pl.col(date_col)).limit(5).collect()
        if sample.is_empty():
            return df

        dtype = sample[date_col].dtype
        dtype_str = str(dtype)

        # Native Date type
        if dtype == pl.Date:
            return df.filter(pl.col(date_col).dt.month().is_in(filter_months))

        # Native Datetime (with or without timezone) — e.g. TIMESTAMPTZ from postgres
        if "Datetime" in dtype_str:
            return df.filter(
                pl.col(date_col).dt.convert_time_zone("UTC").dt.month().is_in(filter_months)
                if "time_zone" in dtype_str and "None" not in dtype_str
                else pl.col(date_col).dt.month().is_in(filter_months)
            )

        # String — detect format from actual sample value
        raw = sample[date_col][0]
        val_str = str(raw).strip() if raw is not None else ""

        if not val_str or val_str == "None":
            return df

        # "2024-04-01 00:00:00" or "2024-04-01 00:00:00+00"
        ...
        # "2024-04-01T00:00:00"
        if len(val_str) >= 19 and val_str[10] == "T":
            return df.filter(
                pl.col(date_col).cast(pl.Utf8)
                .str.slice(0, 10)
                .str.to_date(format="%Y-%m-%d", strict=False)
        ...
        # Last resort: month is chars [5:7] in any YYYY-MM-... format
        month_strs = [str(m).zfill(2) for m in filter_months]
        return df.filter(
            pl.col(date_col).cast(pl.Utf8).str.slice(5, 2).is_in(month_strs)
        )
...
@celery_app.task(bind=True, name="worker.tasks.run_timeseries")
def run_timeseries(
    self, file_path: str, job_id: str, date_col: str, val_col: str,
    dataset_id: str | None = None, filter_months: list[int] | None = None
) -> bytes:
    asyncio.run(update_job_status(job_id, "running"))
    try:
        df = _load_dataset_df(file_path, dataset_id, lazy=True)
        ...
        if filter_months:
            df = _apply_month_filter(df, date_col, filter_months)
...
def run_forecast(
    ...
    try:
        ...
        if filter_months:
            df_lazy = df.lazy()
            df_lazy = _apply_month_filter(df_lazy, date_col, filter_months)
            df = df_lazy.collect()
        ...
        historical = ts_df.with_columns(
            pl.col(date_col).cast(pl.Utf8).str.slice(0, 10).alias(date_col),
            pl.lit("historical").alias("type")
        ).to_dicts()
        combined = historical + forecast_results
        res = pl.from_dicts(combined)
