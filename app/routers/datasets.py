...
#                 "name": col,
#                 "inferred_type": new_type,
#                 # Preserve any user override — never overwrite it on redetect
#                 "user_type": prior.get("user_type"),
#             }
#             new_meta.append(merged)
#             if old_type != new_type:
#                 changed.append({"column": col, "from": old_type, "to": new_type})
...
# @router.post("/datasets/{dataset_id}/override-types", response_model=dict)
# async def override_column_types(
#     dataset_id: UUID,
#     overrides: dict[str, str] = Body(..., embed=True),
#     rls: tuple = Depends(get_user_conn),
# ):
#     """Save user-specified column type overrides permanently to the database.

#     overrides is a dict mapping column name to desired type string.
#     e.g. {"Branch Code": "text", "Invoice Number": "text"}

#     The override is stored as user_type in the columns JSONB.
#     Analysis and profiling always use user_type when present,
#     falling back to inferred_type only when no override exists.
#     """
#     valid_types = {"text", "number", "date", "datetime", "currency", "boolean", "percentage", "id"}
#     for col, typ in overrides.items():
#         if typ not in valid_types:
#             raise HTTPException(
#                 status_code=400,
#                 detail=f"Invalid type for column '{col}'. Must be one of: {sorted(valid_types)}",
#             )

#     rls_pool, owner = rls

#     # Fetch current columns and update using the same connection pattern
#     # as all other endpoints in this file
#     async with rls_conn(rls_pool, owner) as conn:
#         ds = await conn.fetchrow(
#             "SELECT id, columns FROM datasets WHERE id = $1",
#             dataset_id,
#         )
#         if not ds:
#             raise HTTPException(status_code=404, detail="Dataset not found")

#         # Merge overrides into existing column metadata
#         existing_cols: list[dict] = []
#         raw_cols = ds["columns"]
#         if isinstance(raw_cols, str):
#             existing_cols = json.loads(raw_cols)
#         elif isinstance(raw_cols, list):
#             existing_cols = raw_cols
#         else:
#             existing_cols = []

#         updated_cols: list[dict] = []
#         for col_meta in existing_cols:
#             col_name = col_meta.get("name", "")
#             if col_name in overrides:
#                 # Store user override — never erased by re-detect
#                 updated_cols.append({**col_meta, "user_type": overrides[col_name]})
#             else:
#                 updated_cols.append(col_meta)

#         # Persist back to database
#         await conn.execute(
#             "UPDATE datasets SET columns = $1::jsonb WHERE id = $2",
#             json.dumps(updated_cols),
#             dataset_id,
#         )

#     # Return updated columns with effective_type so frontend can update badges
#     result = []
#     for col_meta in updated_cols:
#         result.append({
#             **col_meta,
#             "effective_type": col_meta.get("user_type") or col_meta.get("inferred_type", "text"),
#         })

#     return {"id": str(dataset_id), "columns": result, "overrides_saved": len(overrides)}
    ...
    for r in rows:
        ...
        try:
            ...
            if isinstance(cols, str):
                ...
                cols = _json.loads(cols)
            if cols:
                null_pcts = [float(c.get("null_pct", 0) or 0) for c in cols if isinstance(c, dict)]
                if null_pcts:
                    d["completeness_pct"] = round(100 - sum(null_pcts) / len(null_pcts), 1)
                else:
                    d["completeness_pct"] = None
            else:
                d["completeness_pct"] = None
    ...
    **strategy** controls how tables are combined:
    ...
    async with rls_conn(rls_pool, owner) as conn:
        ...
        if strategy == "union_all":
            ...
            # Preserve left (primary) table column order, extra cols appended after
            left_cols = per_table_cols[0]
            left_set = set(left_cols)
            extra_cols = [c for c in sorted(all_cols) if c not in left_set]
            ordered_cols = left_cols + extra_cols
            ...
            for idx, cnames in enumerate(per_table_cols):
                ...
                for c in ordered_cols:
                    if c in col_set:
                        exprs.append(f'"{c}"::text AS "{c}"')
                    else:
                        exprs.append('NULL::text AS "' + c + '"')
                table = validated[idx]
                selects.append(f"SELECT {', '.join(exprs)} FROM {table}")
            ...
            left_cols_q = await conn.fetch(
                "SELECT column_name FROM information_schema.columns "
                "WHERE table_schema = 'datasets' AND table_name = $1 ORDER BY ordinal_position",
                left_table.split(".")[-1],
            )
            left_cols = [c["column_name"] for c in left_cols_q]
 
            right_cols_q = await conn.fetch(
                "SELECT column_name FROM information_schema.columns "
                "WHERE table_schema = 'datasets' AND table_name = $1 ORDER BY ordinal_position",
                right_table.split(".")[-1],
            )
            right_cols = [c["column_name"] for c in right_cols_q]
 
            left_exprs = [f'l."{c}"::text AS "{c}"' for c in left_cols]
            right_exprs = [f'r."{c}"::text AS "{c}"' for c in right_cols if c != join_key]
            all_exprs = left_exprs + right_exprs
            ordered_cols = left_cols + [c for c in right_cols if c != join_key]
            ...
            join_sql = (
                f"SELECT {', '.join(all_exprs)} "
                f"FROM {left_table} l "
                f"LEFT JOIN {right_table} r "
                f'ON l."{join_key}"::text = r."{join_key}"::text'
            )
            new_table = f"datasets.ds_{uuid_mod.uuid4().hex}"
            await conn.execute(f"CREATE TABLE {new_table} AS {join_sql}")
            row_count = await conn.fetchval(f"SELECT COUNT(*) FROM {new_table}")
