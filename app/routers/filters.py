...
async def _get_or_create_user_id(conn: asyncpg.Connection, email: str) -> UUID:
    """Look up user UUID by email. Create the user row if it doesn't exist yet."""
    row = await conn.fetchrow(
        "SELECT id FROM core_user WHERE email = $1", email
    )
    if row:
        return row["id"]
    # Auto-create the user so the app works without a separate auth setup
    new_id = await conn.fetchval(
        "INSERT INTO core_user (email, name) VALUES ($1, $2) RETURNING id",
        email, email.split("@")[0]
    )
    return new_id
...
async def list_filters(
    ...
    async with pool.acquire() as conn:
        user_id = await _get_or_create_user_id(conn, user)
        if dataset_type:
            rows = await conn.fetch(
                """SELECT * FROM filter_templates
                   WHERE created_by = $1 AND dataset_type = $2
...
async def create_filter(
    ...
    async with pool.acquire() as conn:
        ...
        existing = await conn.fetchrow(
            "SELECT id FROM filter_templates WHERE created_by = $1 AND name = $2",
            user_id, name,
        )
        if existing:
            raise HTTPException(status_code=400, detail="A filter with this name already exists")
        ...
        row = await conn.fetchrow(
            """INSERT INTO filter_templates
               (created_by, name, description, dataset_type, filters,
                sort_by, sort_order, view_columns, filter_count)
               VALUES ($1,$2,$3,$4,$5::jsonb,$6,$7,$8::text[],$9)
               RETURNING *""",
            user_id, name, description, ds_type,
            json.dumps(filters), sort_by, sort_order,
            list(view_cols) if view_cols is not None else None,
            len(filters) if isinstance(filters, list) else 0,
        )
    return _to_dict(row)
...
async def delete_filter(
    ...
    async with pool.acquire() as conn:
        user_id = await _get_or_create_user_id(conn, user)
        result = await conn.execute(
            "DELETE FROM filter_templates WHERE id = $1 AND created_by = $2",
            filter_id, user_id,
        )
    if result == "DELETE 0":
        raise HTTPException(status_code=404, detail="Filter not found")
...
async def duplicate_filter(
    ...
    async with pool.acquire() as conn:
        user_id = await _get_or_create_user_id(conn, user)
        original = await conn.fetchrow(
            "SELECT * FROM filter_templates WHERE id = $1 AND created_by = $2",
            filter_id, user_id,
        )
        if not original:
            raise HTTPException(status_code=404, detail="Filter not found")
        ...
        row = await conn.fetchrow(
            """INSERT INTO filter_templates
               (created_by, name, description, dataset_type, filters,
                sort_by, sort_order, view_columns, filter_count)
               VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *""",
            user_id, new_name, original["description"], original["dataset_type"],
            original["filters"], original["sort_by"], original["sort_order"],
            original["view_columns"], original["filter_count"],
        )
    return _to_dict(row)
